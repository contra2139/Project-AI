...
# Biến toàn cục để quản lý vòng đời ứng dụng Telegram
bot_app = None
...
async def stop_telegram_bot():
    """Dừng Bot Telegram an toàn, đóng các session đang mở"""
    global bot_app
    if bot_app:
        logger.info("🛑 Đang dừng Telegram Bot...")
        try:
            # 1. Dừng updater trước để ngắt polling
            ...
            # 3. Shutdown application hoàn toàn
            await bot_app.shutdown()
            
            # 4. Để lại một khoảng nghỉ cực ngắn cho aiohttp dọn dẹp connector
            await asyncio.sleep(0.2)
            
            logger.info("✅ Telegram Bot shut down successfully.")
        except Exception as e:
            logger.error(f"Lỗi khi dừng Telegram Bot: {e}")
        finally:
            bot_app = None
